...
withFocus ::
  ...
  -> ListZipper a
  -> ListZipper a
withFocus g (ListZipper l x r) = ListZipper l (g x) r
...
findLeft p (ListZipper l x r) =
  let (l1, l2) = break p l in case l2 of
    Nil -> IsNotZ
    (y:.ys) -> IsZ (ListZipper ys y (l1 ++ pure x ++ r))
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight p (ListZipper l x r) = let (r1, r2) = break p r in case r2 of
  Nil -> IsNotZ
  (y:.ys) -> IsZ $ ListZipper (reverse r1 ++ pure x ++ l) y ys
...
moveLeftLoop (ListZipper l x r) = case l of
  Nil -> let (y:.ys) = reverse (x:.r) in ListZipper ys y Nil
  (z:.zs) -> ListZipper zs z (x:.r)
...
moveRightLoop (ListZipper l x r) = case r of
  Nil -> let (y:.ys) = reverse (x:.l) in ListZipper Nil y ys
  (z:.zs) -> ListZipper (x:.l) z zs
...
moveLeft ::
  ListZipper a
  -> MaybeListZipper a
moveLeft z@(ListZipper l _ _) = case isEmpty l of
  True -> IsNotZ
  False -> IsZ (moveLeftLoop z)
...
moveRight ::
  ListZipper a
  -> MaybeListZipper a
moveRight z@(ListZipper _ _ r) = case isEmpty r of
  True -> IsNotZ
  False -> IsZ (moveRightLoop z)
...
-- prop> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end z@(ListZipper l x r) = case reverse r of
  Nil -> z
  (y:.ys) -> ListZipper (ys ++ (x:. l)) y Nil
...
start ::
  ListZipper a
  -> ListZipper a
start z@(ListZipper l x r) = case reverse l of
  Nil -> z
  (y:.ys) -> ListZipper Nil y (ys ++ (x:.r))
...
-- /Tip:/ Use `zipWith`
  (<*>) (ListZipper l x r) (ListZipper l' x' r') =
    ListZipper (zipWith ($) l l') (x x') (zipWith ($) r r')
...
instance Extend ListZipper where
  (<<=) f z = ListZipper (map f zl)  (f z)  (map f zr)
    where zl = unfoldr leftit z
          zr = unfoldr rightit z

leftit :: ListZipper a -> Optional (ListZipper a, ListZipper a)
leftit z = case (isEmpty . lefts) z of
  True -> Empty
  False -> let z' = moveLeftLoop z in Full (z', z')

rightit :: ListZipper a -> Optional (ListZipper a, ListZipper a)
rightit z = case (isEmpty . rights) z of
  True -> Empty
  False -> let z' = moveRightLoop z in Full (z', z')
